...
# %% auto 0
__all__ = ['NOPYTHON', 'rand_choice_nb', 'Forager', 'train_loop_reset', 'run_agents_reset_1D', 'run_agents_reset_2D',
           'train_loop', 'run_single_agent_parallel', 'train_loop_tau', 'run_single_agent_tau_parallel']

# %% ../../../nbs/lib_nbs/12_agents_numba.ipynb 4
import numpy as np
from numba.experimental import jitclass
from numba import float64, int64,  njit, prange, jit
...
# %% ../../../nbs/lib_nbs/12_agents_numba.ipynb 26
NOPYTHON = True #False if you want to have the plain Python run, useful for debugging
...
# from rl_opts.rl_framework.numba.environments import TargetDirectedMotion
# from rl_opts.rl_framework.numba.agents import Forager, train_loop

NOPYTHON = True #False if you want to have the plain Python run, useful for debugging
@jit(nopython = NOPYTHON, parallel = True)
def run_single_agent_parallel(episodes, # Number of episodes
               ...
               num_actions = 4, # From here are props of the agent (see Forager for details)
...
# %% ../../../nbs/lib_nbs/12_agents_numba.ipynb 36
NOPYTHON = True #False if you want to have the plain Python run, useful for debugging
@jit(nopython = NOPYTHON)
def train_loop_tau(episodes : int, # Number of episodes to train
                   time_ep : int, # Length of episode
                   agent : object, # Agent class
                   env : object, # Environment class
                   h_mat_allT : bool = False # If True, returns the h_matrix at all times
    ...
    '''    
    Given an agent and environment, performs a loop train for the `CollectiveSearch` environment, by adequatly
    updating the H and G counters, considering boundaries, etc... 
    '''
...
# %% ../../../nbs/lib_nbs/12_agents_numba.ipynb 41
# from rl_opts.rl_framework.numba.environments import CollectiveSearch
# from rl_opts.rl_framework.numba.agents import Forager, train_loop_tau

NOPYTHON = True #False if you want to have the plain Python run, useful for debugging
@jit(nopython = NOPYTHON, parallel = True)
def run_single_agent_tau_parallel(episodes, # Number of episodes
               time_ep, # Length of episode
               N_agents, # Number of agents               
               h_mat_allT = False, # If to save the h_matrix at all times
    ...
    for n_agent in prange(N_agents):
        ...
        agent = Forager(num_actions,size_state_space,gamma_damping,
                        eta_glow_damping,policy_type,beta_softmax,
                        initial_prob_distr,fixed_policy,max_no_H_update)
        rews, mat = train_loop_tau(episodes, time_ep, agent, env, h_mat_allT)
